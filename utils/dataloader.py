...
from datasets.long_tail import classify_label, train_long_tail, flatten_list, train_long_tail_fmnist
...
def load_fmnist_data(datadir):
    ...
    train_data = mnist_train_ds.data_detailed
    test_data = mnist_test_ds.data_detailed
    lab2cname = mnist_train_ds.lab2cname
    classnames = mnist_train_ds.classnames

    return (X_train, y_train, X_test, y_test, train_data, test_data, lab2cname, classnames)



def load_fmnist_LT_data(datadir, imb_factor, imb_type):
    transform = transforms.Compose([transforms.ToTensor()])
    mnist_train_ds = FashionMNIST_truncated(datadir, train=True, download=True, transform=transform)
    mnist_test_ds = FashionMNIST_truncated(datadir, train=False, download=True, transform=transform)

    list_label2indices = classify_label(mnist_train_ds, 10)

    _, list_label2indices_train_new = train_long_tail_fmnist(copy.deepcopy(list_label2indices), 10, imb_factor, imb_type)

    train_indices = flatten_list(list_label2indices_train_new)

    X_train = mnist_train_ds.data[train_indices]
    y_train = mnist_train_ds.target[train_indices]

    train_data = [mnist_train_ds.data_detailed[i] for i in train_indices]
...
def load_cifar10_LT_data(datadir, imb_factor, imb_type):
    transform = transforms.Compose([transforms.ToTensor()])

    cifar10_train_ds = CIFAR10_truncated(datadir, train=True, download=True, transform=transform)
    cifar10_test_ds = CIFAR10_truncated(datadir, train=False, download=True, transform=transform)

    list_label2indices = classify_label(cifar10_train_ds, 10)

    _, list_label2indices_train_new = train_long_tail(copy.deepcopy(list_label2indices), 10, imb_factor, imb_type)

    train_indices = flatten_list(list_label2indices_train_new)

    X_train = cifar10_train_ds.data[train_indices]
    y_train = cifar10_train_ds.target[train_indices]
    train_data = [cifar10_train_ds.data_detailed[i] for i in train_indices]
...
def load_cifar100_LT_data(datadir, imb_factor, imb_type):
    transform = transforms.Compose([transforms.ToTensor()])

    cifar100_train_ds = CIFAR100_truncated(datadir, train=True, download=True, transform=transform)
    cifar100_test_ds = CIFAR100_truncated(datadir, train=False, download=True, transform=transform)

    list_label2indices = classify_label(cifar100_train_ds, 100)

    _, list_label2indices_train_new = train_long_tail(copy.deepcopy(list_label2indices), 100, imb_factor, imb_type)

    train_indices = flatten_list(list_label2indices_train_new)

    X_train = cifar100_train_ds.data[train_indices]
    y_train = cifar100_train_ds.target[train_indices]
    train_data = [cifar100_train_ds.data_detailed[i] for i in train_indices]
...
def load_celeba_data(datadir):
    ...
    celeba_test_ds = CelebA_custom(datadir, split='test_acpfl', target_type="attr", download=True, transform=transform)
...
def load_femnist_data(datadir):
    ...
    mnist_train_ds = FEMfNIST(datadir, train=True, transform=transform, download=True)
