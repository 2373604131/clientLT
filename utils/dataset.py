...
class FashionMNIST_truncated(data.Dataset):
    ...
    def __init__(self, root, dataidxs=None, train=True, transform=None, target_transform=None, download=False):
        ...
        # 首先定义 classnames
        self.classnames = ['T-shirt/top', 'Trouser', 'Pullover', 'Dress', 'Coat',
        ...
        # 最后构建详细数据
        self.data_detailed = self.__build_detailed_data__()
    ...
    def __build_detailed_data__(self):
        detailed_data = []
        for img, label in zip(self.data, self.target):
            detailed_data.append({
                'image': img,
                'label': label.item(),
                'class_name': self.classnames[label.item()]
            })
...
class SVHN_custom(data.Dataset):
    ...
    def __build_truncated_dataset__(self):
        if self.train is True:
            ...
            svhn_dataobj = SVHN(self.root, 'test_acpfl', self.transform, self.target_transform, self.download)
            data = svhn_dataobj.data
            target = svhn_dataobj.labels
...
class CelebA_custom(VisionDataset):
    ...
    Args:
        ...
        split (string): One of {'train', 'valid', 'test_acpfl', 'all'}.
