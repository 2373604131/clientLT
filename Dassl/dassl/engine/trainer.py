    ...
    def train(self, start_epoch, max_epoch, idx=-1,global_epoch=-1,is_fed=False,global_weight=None, fedprox=False, mu=0.5):
        ...
        for self.epoch in range(self.start_epoch, self.max_epoch):
            if self.sched is not None:
                if not getattr(self, "_skip_scheduler_step_at_epoch_start", False):
                    self.sched.step()
            self.before_epoch()
...
class SimpleTrainer(TrainerBase):
    ...
    def global_test(self, split=None, is_global=False, current_epoch=0, idx=-1):
        ...
        #     # 使用到其他类别的平均距离的倒数作为权重
        #     weights = 1 / (distance_matrix[cls] + 1e-5)  # 添加小值避免除零
        #     weights[cls] = 0  # 忽略自身
        #     inverse_distance = weights.sum()
        #     estimated_distribution[cls] = inverse_distance
        #     total_inverse_distance += inverse_distance
        #
        # # 归一化估计的分布
        # total = sum(estimated_distribution.values())
        # for cls in estimated_distribution:
        #     estimated_distribution[cls] /= total
        #
        # # 打印估计的分布和实际的类别计数
        # print("\nEstimated class distribution vs Actual class counts:")
        # for cls in sorted(estimated_distribution.keys()):
        #     est_prob = estimated_distribution[cls]
        #     print(f"Class {cls}: Estimated prob = {est_prob:.4f}")
##################################################################
        #         probs = self.model(input, return_prob=True)
        #         for prob, label in zip(probs, labels):
        #             # 确保 prob 是 float 类型，label 是能作为字典键的类型
        #             prob = float(prob)
        #             label = label.item() if isinstance(label, torch.Tensor) else label
        #
        #             inverse_probs[label] += 1 / (prob + 1e-5)  # 加小值避免除零
        #             class_counts[label] += 1
        #
        # # 归一化反比例概率
        # total_inverse_prob = sum(inverse_probs.values())
        # estimated_distribution = {cls: prob / total_inverse_prob for cls, prob in inverse_probs.items()}
        # print("\nEstimated class distribution vs Actual class counts:")
        # sortable_keys = [k.item() if isinstance(k, torch.Tensor) else k for k in estimated_distribution.keys()]
        #
        # for cls in sorted(sortable_keys):
        #     original_key = torch.tensor(cls) if isinstance(next(iter(estimated_distribution.keys())),
        #                                                    torch.Tensor) else cls
        #     est_prob = estimated_distribution[original_key]
        #     print(f"Class {cls}: Estimated prob = {est_prob:.4f}")
##################################################################
    ...
    def global_tsne(self,current_epoch):
        self.set_model_mode("eval")
        self.evaluator.reset()
        ...
        all_labels = []
        perplexities = [6]
        save_path = f'output/plot/tsne_cifar10_{current_epoch}.pdf'
        with torch.no_grad():
            for batch_idx, batch in enumerate(tqdm(data_loader)):
                input, labels = self.parse_batch_test(batch)
                image_features, output, _ = self.model(input, return_features=True)
        ...
        # CIFAR-10的颜色映射
        PALETTE = [
            "#17BECF",  # 0: airplane - 柔和的蓝色
            "#1F77B4",  # 1: automobile - 淡橙色
            "#8C564B",  # 2: bird - 柔和的红色
            "#FF7F0E",  # 3: cat - 青绿色
            "#BCBD22",  # 4: deer - 森林绿
            "#9467BD",  # 5: dog - 柔和的黄色
        ...
        # 创建画布
        fig, axs = plt.subplots(1, len(perplexities), figsize=(10.2 * len(perplexities), 10))
        if len(perplexities) == 1:
            axs = [axs]

        # 为每个perplexity值创建一个t-SNE图
        for row, perplexity in enumerate(perplexities):
            ...
            # 绘制散点图
            scatter = axs[row].scatter(tsne_result[:, 0], tsne_result[:, 1],
                                       c=all_labels, cmap=color_map,
                                       alpha=0.7, s=50)

            # 设置坐标轴
            axs[row].set_xticklabels([])
            axs[row].set_yticklabels([])
            axs[row].set_xlabel('t-SNE 1', fontsize=27)
            axs[row].set_ylabel('t-SNE 2', fontsize=27)
            ...
            axs[row].legend(handles=legend_elements, 
                           loc='center left', 
        ...
        print(f"Saved image feature t-SNE visualization to {save_path}")
        # plt.show()
        plt.close()
