...
def build_data_loader(
    ...
    if cfg.DATASET.NAME == "Cifar100" or cfg.DATASET.NAME == "Cifar10" or cfg.DATASET.NAME == "Cifar10_LT" or cfg.DATASET.NAME == "Cifar100_LT":
        dataset_wrapper = DatasetCifar100
    elif cfg.DATASET.NAME == "FashionMNIST" or cfg.DATASET.NAME == "FashionMNIST_LT":
        dataset_wrapper = DatasetFMNIST
    elif cfg.DATASET.NAME == "ImageNet_LT":
        dataset_wrapper = DatasetImageNet
...
class DataManager:
    ...
    def __init__(
        ...
        # Fed
        self.client_proportion = dataset.client_proportions
        if cfg.VERBOSE:
    ...
    def show_dataset_summary(self, cfg):
        ...
        if self.dataset.federated_test_x:
            ...
            table.append(["# test_acpfl", f"{len(self.dataset.test):,}"])
...
class DatasetImageNet(TorchDataset):
...
class DatasetWrapper_TextOnly(TorchDataset):
    ...
    def __init__(self, cfg, class_names):
        ...
        if cfg.TRAINER.PROTEXT.GPT_PATH != "":
            ...
            k = 0
            # print(f"Number of keys in GPT_prompt_dict: {len(GPT_prompt_dict)}")
            for id, single_key in enumerate(GPT_prompt_dict.keys()):
                ...
                if single_key_formatted in name_to_id:
                    # print(f"Adding data for key: {single_key_formatted}")
    ...
    def __getitem__(self, idx):
        ...
        if self.transform is not None:
            if isinstance(self.transform, (list, tuple)):
                ...
                if self.cfg.DATASET.NAME == "Cifar100" or self.cfg.DATASET.NAME == 'Cifar100_LT':
                    transform = T.Compose([
                        T.ToTensor(),
                    ...
                    img = transform(img0)
                elif self.cfg.DATASET.NAME == "Cifar10" or self.cfg.DATASET.NAME == 'Cifar10_LT':
                    transform = T.Compose([
                        T.ToTensor(),
...
class DatasetFMNIST(TorchDataset):
    def __init__(self, cfg, data_source, transform=None, is_train=False):
        self.cfg = cfg
        self.data_source = data_source
        self.is_train = is_train
        ...
        self.transform = T.Compose([
            T.ToPILImage(),
            T.Resize((224, 224)),
            T.Grayscale(3),  # 将单通道图像转换为3通道
            T.ToTensor(),
            T.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))  # 适用于3通道图像的标准化
        ])
    ...
    def __getitem__(self, idx):
        ...
        if isinstance(img0, torch.Tensor):
            if img0.dim() == 1:
                img0 = img0.view(28, 28)  # FMNIST图像是28x28
        else:
            img0 = torch.tensor(img0).view(28, 28)

        img = self.transform(img0)
        output["img"] = img

        if self.return_img0:
            output["img0"] = T.ToTensor()(img0)
