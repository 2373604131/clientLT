...
class ModifiedResNet_CAPT(nn.Module):
    def __init__(self, layers, output_dim, heads, input_resolution=224, width=64):
        super().__init__()
        self.output_dim = output_dim
        self.input_resolution = input_resolution
        ...
        # 视觉提示投影层
        # self.prompt_proj = nn.Linear(2048, embed_dim)  # 假设输入的视觉提示维度为2048
    ...
    def forward(self, x, vision_prompts):
        def stem(x):
            for conv, bn in [(self.conv1, self.bn1), (self.conv2, self.bn2), (self.conv3, self.bn3)]:
                x = self.relu(bn(conv(x)))
            x = self.avgpool(x)
        ...
        # 处理视觉提示
        projected_prompts = vision_prompts
        projected_prompts = projected_prompts.view(projected_prompts.size(0), -1, 1, 1)
        projected_prompts = projected_prompts.expand(-1, -1, x.size(2), x.size(3))

        # 将视觉提示添加到特征图
        x = x + projected_prompts

        x = self.attnpool(x)

        # 确保输出是2D张量 [batch_size, feature_dim]
        if x.dim() > 2:
            x = x.mean(dim=1)  # 或者使用 x = x[:, 0, :] 如果您想使用第一个token
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, text_layer=False, design_details=None):
        ...
        print("current_trainer",current_trainer)
        if current_trainer == 'MaPLe':
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_MaPLe(width, heads, attn_mask, design_details)
...
class VisionTransformer_CAPT(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int,
                 design_details):
        super().__init__()
        self.input_resolution = input_resolution
    ...
    def forward(self, x: torch.Tensor, shared_ctx):
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        x = torch.cat(
        ...
        x = x.permute(1, 0, 2)  # NLD -> LND
        # 直接使用 transformer，不传入 compound_deeper_prompts
        x = self.transformer(x)
        x = x.permute(1, 0, 2)  # LND -> NLD
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            if trainer == "CAPT":
                self.visual = ModifiedResNet_CAPT(
            ...
            if trainer == "CAPFL":
                ...
                self.visual = VisionTransformer_CAPT(
                    input_resolution=image_resolution,
                    patch_size=vision_patch_size,
                    width=vision_width,
                    layers=vision_layers,
