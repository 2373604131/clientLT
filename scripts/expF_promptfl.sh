...
for DATASET in ${DATASETS}; do
  ...
  for SEED in ${SEEDS}; do
    ...
    for BETA in ${DIRICHLET_BETAS}; do
      ...
      DIR="${BASE_OUTPUT_DIR}/partition=${PARTITION}_beta=${BETA}_IF=${IMB_FACTOR}_localE=${LOCAL_EPOCHS}_seed=${SEED}"
      CMD=(
      ...
      run_expf_condition "Dirichlet" "beta" "${BETA}" "${SEED}" "${DIR}" "${SCHEDULE_FILE}" "${CMD[@]}"
    done

    for LAMBDA in ${CLIENTLT_LAMBDAS}; do
      PARTITION="client-longtail"
      DIR="${BASE_OUTPUT_DIR}/partition=${PARTITION}_lambda=${LAMBDA}_alpha=${INTRA_GROUP_ALPHA}_rho=${HEAD_LEAKAGE_SCALE}_IF=${IMB_FACTOR}_localE=${LOCAL_EPOCHS}_seed=${SEED}"
      CMD=(
        "${PYTHON_BIN}" federated_main.py
        --root "${DATA}"
        --model "${MODEL}"
      ...
      run_expf_condition "Client-LT" "lambda" "${LAMBDA}" "${SEED}" "${DIR}" "${SCHEDULE_FILE}" "${CMD[@]}"
    done
  done
done
