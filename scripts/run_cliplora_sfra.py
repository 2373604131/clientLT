"""Foreground SFRA V1 runs, resumption, and lightweight result collection."""
import argparse
import json
import math
import os
...
def prepare_protocol(args, run):
    ...
    for name in required:
        if not (source/name).is_file():
            raise FileNotFoundError(f'{source/name}; provide --reference-run with the full S run, '
...
def main():
    ...
    parser.add_argument('--method', choices=['s', 'current', 'full', 'flat'], default='full')
    parser.add_argument('--retention-weight', type=float, default=10., help='Only proposed method-strength search parameter')
    ...
    parser.add_argument('--output-root', type=Path, default=Path('output/cifar100_LT/sfra_v1'))
    parser.add_argument('--bridge-root', type=Path, default=Path('output/cifar100_LT/a_refresh_topology_bridge'))
    parser.add_argument('--reference-run', type=Path, help='Replay the exact partition/order/schedule of an existing S run')
    ...
    parser.add_argument('--resume', action='store_true', help='Resume this method/lambda directory at its last completed round')
